...
public class WekaWrapper {
	...
	public Instances applyCostMatrix(Instances dataset, String costmatrix){
		CostMatrix matrix;
	...
	public void SaveInstances(Instances dataset, String file) throws IOException{
	...
	public Instances LoadData(String data) throws Exception{
	...
	public void GetPridictionDistribution(Classifier classifier, Instances testset, String output) throws Exception{
	...
	public void GetPridiction(Classifier classifier, Instances testset, String output) throws Exception{
		//output the labels
		BufferedWriter writer = new BufferedWriter(new FileWriter(output));
	...
	public void TrianTest(Instances trainset, Instances testset) throws Exception{
		...
		for(int i = 0; i < classifiers.length; i++)
		{
			...
			if(labelfile == ""){
				labelfile = "test.label.arff";
			}
			GetPridiction(classifier, testset, labelfile);
			//GetPridictionDistribution(classifier, testset, test + ".dis.label");
		}
	}
		
	public void TrianTest(String train, String test) throws Exception{
	...
	public Instances PerformPCA(Instances dataset, int k) throws Exception{
	...
	public Instances AddWeight(Instances dataset, String weightfile) throws Exception{
	...
	public void Crossvalidataion(Instances dataset, int folder, Boolean output) throws Exception
	{
		Boolean OutputPredictionFlag = output;
		
		System.out.println(dataset.numInstances());
		
		//Test Feature Selection
		//dataset = this.FeatureSelection(dataset);
		...
		for(int i = 0; i < classifiers.length; i++)
		{
			...
			if(OutputPredictionFlag){
				...
				Pattern pattern = Pattern.compile("\\s*(\\d+)\\s+\\d+:([^\\s-]+)\\s+\\d+:([^\\s-]+).*\\(([^\\s-]+)\\)");
				...
				BufferedWriter writer = new BufferedWriter(new FileWriter(this.labelfile));
				writer.write("True" + "\t" + "Predict" + "\t" + "fold");
				writer.newLine();
				
				for(ArrayList<String> predict: predictions){
					writer.write(predict.get(0) + "\t" + predict.get(1)+ "\t" + predict.get(3));
					writer.newLine();
				}
	...
	public void Crossvalidataion(String data, int folder, Boolean output) throws Exception
	{
		Instances dataset = LoadData(data);
		System.out.print(data + "\t");
		Crossvalidataion(dataset, folder, output);
	}
